...
module.exports = wann = async (rifza, riff) => {
	try {
        if (!wan.hasNewMessage) return
        riff = wan.messages.all()[0]
...
*[Wait] Tunggu Sebentar..*`
				...
				wann.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:ftroli, caption: 'Nih Gan'})
					break               	        	 	        	 	        	
...
*[Wait] Tunggu Sebentar..*`				
				...
				wann.sendMessage(from, buffer1, audio, {mimetype: 'Mimetype.mp4Audio', Quality: `${anu.data.sd.quality}.mp4`, quoted:ftroli, caption: 'Nih Gan'})
					break							
...
if (isBanchat) return
              ...
              if (!c) return reply('Linknya?')
              if (!c.includes('tiktok')) return reply(mess.error.Iv)
        sendButMessage(from, `[ TIKTOK DOWNLOAD ]`, `Silahkan pilih salah satu`, [
          {buttonId: `${prefix}infott ${c}`, buttonText: {displayText: '📃INFORMATION'}, type: 1},
   {buttonId: `${prefix}tiktokvideo ${c}`, buttonText: {displayText: '▶️️VIDEO'}, type: 1},
   {buttonId: `${prefix}downloadmenu ${c}`, buttonText: {displayText: '⬅️Back to menu'}, type: 1},
        ]);
        break;      
...
if (isBanchat) return   
				...
				sendMediaURL(from, anu.result.audio)
					break
	case 'tiktokaudio2':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
		...
		hx.ttdownloader(`${c}`)
    		...
    		.then(async (a) => {
    		me = `*Link* : ${a.data}`
		wann.sendMessage(from,{url:`${nowm}`},audio,{mimetype:'Mimetype.mp4Audio',quoted:riff,caption:me})
		})
		})
		break				
	 case 'tiktokvideo':
...
if (isBanchat) return
		...
		ini_url = args[0]
		 		reply(mess.wait)
                    ...
                    await wann.sendMessage(from, ini_img , image, { quoted: riff, caption: ini_txt })
                    ini_buffer = await getBuffer(get_result.audio)
                    wann.sendMessage(from, ini_buffer, audio, {mimetype: 'audio/mp4', filename: `${get_result.caption}.mp3`, quoted: riff })
            		break               	        	 	        	 	        	 	        	 	        	 	        	
      case 'igstory':
if (isBanned) return reply(mess.ban)
if (isBanchat) return 
           if(!c) return reply('Usernamenya?')
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    wann.sendMessage(from,link,video,{quoted: riff,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    wann.sendMessage(from,link,image,{quoted: riff,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break    
...
`
              wann.sendMessage(from, pp_tt, image, { quoted: riff, caption: ini_txt, thumbnail: Buffer.alloc(0) })
              break
