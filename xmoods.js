...
za_api = "ApiRifzaBot"
LolApi = "Deff" //Ganti sama Api lu
zenzapi = "LordRifza"
xteamkey = "7415bc4287ad5ca8"
target = "6287708357324@s.whatsapp.net" //Ganti jadi nomor lu
...
module.exports = wann = async (wann, wan) => {
	try {
        ...
        var pes = (type === 'conversation' && wan.message.conversation) ? wan.message.conversation : (type == 'imageMessage') && wan.message.imageMessage.caption ? wan.message.imageMessage.caption : (type == 'videoMessage') && wan.message.videoMessage.caption ? wan.message.videoMessage.caption : (type == 'extendedTextMessage') && wan.message.extendedTextMessage.text ? wan.message.extendedTextMessage.text : ''		
        ...
        const conts = wan.key.fromMe ? wann.user.jid : wann.contacts[sender] || { notify: jid.replace(/@.+/, '') }
...
remoteJid :"6289523258649-1604595598@g.us" }: {})
        ...
        const reply1 = (teks) => {
			wann.sendMessage(from, teks, text, { thumbnail: Rfake, sendEphemeral: true, quoted: riff, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6287708357324`}}})
		}
		const reply = (teks) => {
			wann.sendMessage(from, teks, text, { thumbnail: Rfake, sendEphemeral: true, quoted: riff, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"JANGAN DI PENCET",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6287708357324?text=Halo Tuan`}}})
		}
        ...
        const textImg = (teks) => {
           return wann.sendMessage(from, teks, text, {quoted: riff, thumbnail: fs.readFileSync('./gambar/thumb.jpg')})
        }
...
const sendButDocument = async(id, text1, desc1, media, doc1, but = [], options = {}) => {
	...
	wann.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
      ...
      const sendWebp = async(to, url) => {
           ...
           wann.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:riff})
           fs.unlinkSync(asw)
});
});
});
...
const sendButMessage = (id, text1, desc1, but = [], options = {}) => {
      ...
      wann.sendMessage(
       ...
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           wann.sendMessage(to, media, type, {quoted: riff, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
...
//--------------------[AFK]--------------------\\
	if (isGroup) {
		if (!wan.key.fromMe && banChats === true) return
...
//--------------------[ANTILINK]--------------------\\ 
      ...
      if (isGroup && isAntiLink && !isOwner && isBotGroupAdmins){
            ...
            if (isGroupAdmins) return reply("Admin mah bebas ye kan😎");
                setTimeout( () => {
		wann.groupRemove(from, [sender]).catch((e) => { wann.reply(from, global.db.mess.BotAdmin, riff) })
		}, 7000)
        setTimeout( () => {
...
if (isGroup && isAntivirtex && !wan.key.fromMe) {
      if (budy.length > 1200) {
        if (isGroupAdmins) return reply("Admin mah bebas ye kan😎");
...
if (isSimi) {		
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=ID`)
                     sami = simi.success
                        wann.sendMessage(from, `${sami}`, text, {thumbnail: ppbot, sendEphemeral: true, quoted:wan, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
//=====================================================================
...
if (isSimi) {		
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=ID`)
                     sami = simi.success
                        wann.sendMessage(from, `${sami}`, text, {thumbnail: ofrply, sendEphemeral: true, quoted:riff, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
//--------------------[SIMI]--------------------\\                      
...
©Created by Rifza`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
wann.sendMessage(from,  buttonMessagee, MessageType.buttonsMessage,{
        ...
        "contextInfo": {
          ...
          break
case 'jeoehejeheke':
if (isOwner && wan.key.fromMe) return             
              if (args[0] === 'enable'){
...
if (isBanned) return reply(mess.ban)
if (isBanchat) return
if (args.length < 1) return wann.sendMessage(from, `Kode bahasanya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: riff })
const gtts = require('./lib/gtts')(args[0])
if (args.length < 0) return wann.sendMessage(from, `Teksnya mana kak? contoh : ${prefix}tts id yamate kudasai`, text, { quoted: riff })
...
if (isBanchat) return                     
              ...
              prep = await wann.prepareMessageFromContent(from,{buttonsMessage},{quoted: riff})
              wann.relayWAMessage(prep)
              fs.unlinkSync(`./${sender}.jpeg`)
              break
...
ini_result = await fetchJson(`https://api.lolhuman.xyz/api/github/${username}?apikey=WannOff`)
...
case 'asupantiktok': case 'ptl': case 'ptlvid':{
                if (!isPremium && !isOwner && !wan.key.fromMe) return sendButMessage (from, prem1, prem2, prem3, { quoted: freply})
               reply(mess.wait)
...
ini_buffer = await getBuffer(`https://api.lolhuman.xyz/api/  tools/ssweb?url=${ini_link}&apikey=WannOff`)
...
🐤 Character : ${get_info.characters.join(", ")}\n`
       ...
       case 'nhentai':
            ...
            case 'fuckboyserti':
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/fuckboy?apikey=${setting.lolkey}&name=${addit}`)
				wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/fuckgirl?apikey=${setting.lolkey}&name=${addit}`)
				wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/bucinserti?apikey=${setting.lolkey}&name=${addit}`)
				wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				dap1 = ct.split('|')[0]
				dap2 = ct.split('|')[1]
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/pacarserti?apikey=${setting.lolkey}&name1=${dap1}&name2=${dap2}`)
				wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/goodboy?apikey=${setting.lolkey}&name=${addit}`)
				wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/goodgirl?apikey=${setting.lolkey}&name=${addit}`)
				wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/badboy?apikey=${setting.lolkey}&name=${addit}`)
wann.sendMessage(from, aditty, image, {quoted: freply})
				...
				addit = args.join(' ')
				aditty = await getBuffer(`https://api.lolhuman.xyz/api/badgirl?apikey=${setting.lolkey}&name=${addit}`)
			wann.sendMessage(from, aditty, image, {quoted: freply})
       ...
       case 'manga':
              ...
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/manga?apikey=WannOff&query=${query}`)
...
case 'ttaudio': 
  ...
  anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/facebook-dl?url=${q}&apikey=WannOff`)
...
if (isBanchat) return
               ...
               wann.sendMessage(from, buff, image, {quoted: riff, caption: mess.success}).catch(e => {
...
wann.sendMessage(from, photooxy2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'googletxt':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, googletxt2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'pubg':
case 'battlefield':
if (isBanned) return reply(mess.ban)
...
wann.sendMessage(from, game2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'neon':
case 'glow':
if (isBanned) return reply(mess.ban)
...
wann.sendMessage(from, metalic2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'golden':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, roses2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'wooden':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, random2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'textburn':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, alam2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'coffeecup':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, makell2, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
...
wann.sendMessage(from, maker3d22, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'maker3d3':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, maker3d32, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'maker3d4':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
wann.sendMessage(from, maker3d42, image, {quoted:riff,caption:`${q}`,thumbnail:Rfake})
break
case 'kick':
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
if (isBanned) return reply(mess.ban)
if (isBanchat) return
if (!wan.key.fromMe && !isGroupAdmins) return reply("Admin Group Only");
if (!isGroup) return;
...
if (isBanned) return reply(mess.ban)
if (isBanchat) return
if (isOwner && wan.key.fronMe) return  reply(mess.only.owner)
...
if (isBanned) return reply(mess.ban)
if (isBanchat) return
            if (!wan.key.fromMe && !isOwner && !isGroupAdmins) return reply(mess.admin)
            pe = args.join('')
            entah = pe.split('|')[0]
            nah = pe.split('|')[1]
...
wann.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt: true, duration: 359996400, quoted:riff})
fs.unlinkSync(ran)
})
break
...
wann.sendMessage(from, ibb, image, { quoted: riff, caption: caps})
})
.catch(err => {
throw err
})
...
case 'toimg':      
              ...
              wann.sendMessage(from, buffer, image, {quoted: riff, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
...
if (isBanchat) return        
               ...
               wann.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: riff, caption: mess.success})
...
if (isBanchat) return        
               ...
               a = await webp2gifFile(mediaaa)
               mp4 = await getBuffer(a.result)
               wann.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: riff, caption: mess.success})
               fs.unlinkSync(mediaaa)
               } else {
               reply(mess.wrongFormat)
}
...
if (isBanchat) return		
					...
					wann.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: riff})
...
hah = fs.readFileSync(ran)
wann.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: riff})
fs.unlinkSync(ran)
})
break
case 'gemuk':
...
if (isBanned) return reply(mess.ban)
if (isBanchat) return
if (!isOwner && !wan.key.fromMe) return reply(mess.only.ownerB)
...
if (isBanned) return reply(mess.ban)
if (isBanchat) return
       if (!isGroupAdmins && !isOwner && !wan.key.fromMe) return reply(mess.only.admin)
        if (
          ((isMedia && !wan.message.videoMessage) || isQuotedSticker) &&
          args.length == 0
        ) {
          ...
          ((isMedia && !wan.message.videoMessage) || isQuotedImage) &&
          args.length == 0
        ) {
          ...
          ((isMedia && !wan.message.videoMessage) || isQuotedAudio) &&
          args.length == 0
        ) {
          ...
          ((isMedia && !wan.message.videoMessage) || isQuotedVideo) &&
          args.length == 0
        ) {
...
if (isBanchat) return
            if (!isGroup) return reply(mess.only.group)
                cr = body.slice(4)
                cs = cr.split('|')
                taged = wan.message.extendedTextMessage.contextInfo.mentionedJid[0]
                const target = {
					contextInfo: {
						participant: taged,
						quotedMessage: {
...
wann.sendMessage(from, buffer, image, {caption: "Nih", quoted: riff })
...
buffer = await getBuffer(linkpp)
wann.sendMessage(from, buffer, image, { quoted: riff, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
...
case 'ban':
case 'banned':
if (!isOwner && !wan.key.fromMe) return reply(mess.own)
bnnd = body.slice(5)
...
teks += `\n*Total : ${ban.length}*`
wann.sendMessage(from, teks.trim(), extendedText, { quoted: riff, contextInfo: { "mentionedJid": ban } })
break
case 'level': 
if (isBanned) return reply(mess.ban)
if (isBanchat) return
...
*/                             
         if (budy.includes(`bot`)) {
                ...
                wann.sendMessage(from, bot, MessageType.audio, {quoted: ftroli, mimetype: 'audio/mp4', ptt:true})
                  }  
         if (budy.includes(`Bot`)) {
                const bot = fs.readFileSync('./database/vn/xmoods.mp3');
                wann.sendMessage(from, bot, MessageType.audio, {quoted: ftroli, mimetype: 'audio/mp4', ptt:true})
...
• Pemenangnya adalah [ @${winR} ] `
	   wann.sendMessage(from, starGame, text, {quoted: riff, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  wann.sendMessage(from, ucapan, text, {quoted: riff, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
wann.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:riff,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
wann.sendMessage(from, ucapan1, text, {quoted: riff, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
wann.sendMessage(from, ucapan1, text, {quoted:riff, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
